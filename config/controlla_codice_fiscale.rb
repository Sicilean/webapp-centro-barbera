...
class ControllaCodiceFiscale
  ...
  def self.valid?(cf, strict = false)
    if  cf.nil?
      ...
      #return "codice fiscale non può essere lasciato in bianco" if cf.empty?
      #raise InvalidLength.new("codice fiscale dev'essere composto da 16 caratteri alfanumerici") if cf.size != 16
      #return "codice fiscale dev'essere composto da 16 caratteri alfanumerici" if cf.size != 16
      #if strict==true && cf != cf.upcase; return "i caratteri del codice fiscale devono essere maiuscoli"; else cf.upcase! end;
